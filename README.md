...
This project setup uses [Webpack](https://webpack.github.io/) for handling all assets. Webpack offers a custom way of ??œextending??? the concept of `import` beyond JavaScript. To express that a JavaScript file depends on a CSS file, you need to **import the CSS from the JavaScript file**:
...
render() {
  ...
  // as described in ??œAdding Images and Fonts??? above this section.
...
The watcher includes an interactive command-line interface with the ability to run all tests, or focus on a search pattern. It is designed this way so that you can keep it open and enjoy fast re-runs. You can learn the commands from the ??œWatch Usage??? note that the watcher prints after every run:
...
You can also use [`jest.fn()` and `expect(fn).toBeCalled()`](http://facebook.github.io/jest/docs/api.html#tobecalled) to create ??œspies??? or mock functions.

### Testing Components

There is a broad spectrum of component testing techniques. They range from a ??œsmoke test??? verifying that a component renders without throwing, to shallow rendering and testing some of the output, to full rendering and testing component lifecycle and state changes.
...
```sh
    ...
    ?œ”  Database Rules for example-app-fd690 have been downloaded to database.rules.json.
    ...
    ?œ”  Wrote build/index.html

    i  Writing configuration info to firebase.json...
    i  Writing project information to .firebaserc...

    ?œ”  Firebase initialization complete!
```
...
```sh
    ...
    ?œ”  database: rules ready to deploy.
    i  hosting: preparing build directory for upload...
    Uploading: [==============================          ] 75%?œ”  hosting: build folder uploaded successfully
    ?œ”  hosting: 8 files uploaded successfully
...
If you have ideas for more ??œHow To??? recipes that should be on this page, [let us know](https://github.com/facebookincubator/create-react-app/issues) or [contribute some!](https://github.com/facebookincubator/create-react-app/edit/master/packages/react-scripts/template/README.md)
=======
# react-ajax-tutorial
